...
ストリーミング中の軽量1パス解析と、フェンス確定後の詳細解析を確認するためのテスト文書です。
...
fn load_user(id: u64) -> Result<User, String> {
    let mut cache = HashMap::new();
    // multiline comments are re-highlighted after the fence closes.
    /*
       The streaming lexer still colors this safely as comment text.
    */
    for retry in 0..MAX_RETRIES {
        println!("loading user {id}, retry={retry}");
        if id > 0x10 {
            return Ok(User { id, name: "Alice".to_owned() });
        }
    }
...
```javascript
export async function fetchItems(url, limit = 20) {
...
/* This block becomes a detailed comment after the closing fence. */
const pattern = /^(item|entry)-\d+$/;
console.log("matched:", pattern.test("item-42"));
```
...
def distance(a: Point, b: Point) -> float:
    # Unicode and escaped strings should remain intact.
    message = "point\n\tvalue"
...
async fn run() -> Result<(), JsValue> {
    ...
    if search.contains("renderer=canvas2d") {
        let (tps, repeats, auto_scroll, _, fade_ms) = runtime_config();
        return canvas2d::start(canvas, selected_mock(), tps, repeats, auto_scroll, fade_ms);
    }
    let app = match App::new(canvas.clone()).await {
        Ok(app) => Rc::new(RefCell::new(app)),
        Err(error) => {
            web_sys::console::warn_1(
                &format!("GPU renderer unavailable; using Canvas2D: {error:?}").into(),
            );
            let (tps, repeats, auto_scroll, _, fade_ms) = runtime_config();
            return canvas2d::start(canvas, selected_mock(), tps, repeats, auto_scroll, fade_ms);
        }
    ...
    let down = Closure::<dyn FnMut(MouseEvent)>::new(move |event: MouseEvent| {
        ...
        if app.handle_selection_action(&event) {
            return;
        }
...
impl App {
    async fn new(canvas: HtmlCanvasElement) -> Result<Self, JsValue> {
        ...
        let force_webgl = web_sys::window()
            ...
            .is_some_and(|search| search.contains("renderer=webgl"));
        ...
        let adapter = instance
            ...
            .await
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        let backend_name = format!("{:?}", adapter.get_info().backend).to_uppercase();
    ...
    fn frame(&mut self, now: f64) {
        ...
        if self.copied_until > 0.0 && now >= self.copied_until {
            self.copied_until = 0.0;
            self.dirty_scene = true;
        }
        let webgl_throttled = self.backend_name.contains("GL")
            && self.instance_count > 0
            && now - self.last_scene_time < 33.0;
        if self.dirty_scene && !webgl_throttled {
            self.rebuild_scene();
    ...
    fn update_cursor(&self, event: &MouseEvent) {
        ...
        let cursor = if !inside {
            ...
            let on_selection_button = self.selection_range().is_some()
                && (hit(x, y, 18.0, self.config.height as f32 - 50.0, 92.0, 34.0)
                    || hit(x, y, 116.0, self.config.height as f32 - 50.0, 74.0, 34.0));
            if on_selection_button {
                "pointer"
            } else if self.pointer_on_scrollbar(event) {
                "grab"
            } else if self.pointer_on_text(event) {
                "text"
            } else {
                "default"
            }
    ...
    fn handle_selection_action(&mut self, event: &MouseEvent) -> bool {
        ...
        let x = event.client_x() as f32 - rect.left() as f32;
        let y = event.client_y() as f32 - rect.top() as f32;
        let top = self.config.height as f32 - 50.0;
        if hit(x, y, 18.0, top, 92.0, 34.0) {
            return self.copy_selection();
        }
        if hit(x, y, 116.0, top, 74.0, 34.0) {
    ...
    fn copy_selection(&mut self) -> bool {
        ...
        self.copied_until = self.last_time + 1_200.0;
        self.dirty_scene = true;
        true
    }
    ...
    fn pointer_on_text(&self, event: &MouseEvent) -> bool {
        let (x, y) = self.pointer_canvas_position(event);
        y >= 68.0
            && x < self.content_width()
            && self.text_cells.iter().any(|cell| {
                let (cell_x, cell_y, width, height) = self.text_cell_screen_geometry(cell);
                hit(x, y, cell_x, cell_y, width.max(2.0), height)
            })
    }
    ...
    fn sync_layout(&mut self, delta: &Delta) {
        let changed = delta
            ...
            .map(|op| match op {
                ...
                Op::SpliceCode { block, .. } | Op::SealCode { block } => *block as usize,
    ...
    fn rebuild_scene(&mut self) {
        ...
        scene.fast_coverage = self.backend_name.contains("GL");
        ...
        for index in first..last {
            ...
            scene.block(block, self.font_scale);
        }
        scene.finish_block();
        ...
        let status = format!(
            ...
            self.tps,
            self.repeats,
            if self.auto_scroll { "AUTO" } else { "FREE" },
        );
        let status_width: f32 = status.chars().map(|c| font::advance(c, 1.0)).sum();
        let sx = (self.config.width as f32 - status_width - 24.0).max(180.0);
        ...
        if let Some((start, end)) = self.selection_range() {
            let count = scene
                ...
                .iter()
                .filter(|cell| cell.pos >= start && cell.pos < end)
                .map(|cell| cell.text.char_count())
                .sum();
            scene.selection_toolbar(
                self.config.height as f32,
                count,
                self.copied_until > self.last_time,
            );
        }
...
impl Scene {
    ...
    fn code_text(
        ...
        let limit = self.width - 30.0;
        self.snap_text = true;
        code::highlight(source, language, detailed, |span, kind| {
            ...
            for c in span.chars() {
                let glyph = font::mono_glyph(c, scale);
                let advance = glyph.advance;
                if c == '\n' || (x + advance > limit && x > origin) {
                    x = origin;
                    y += line;
    ...
    fn draw_glyph(
        ...
        for py in 0..glyph.height {
            ...
            while px < glyph.width {
                let coverage = |value: u8| {
                    if self.fast_coverage {
                        (((value as u16 + 16) / 32) * 32).min(255) as u8
                    } else {
                        value
                    }
                };
    ...
    fn selection_toolbar(&mut self, viewport_height: f32, count: usize, copied: bool) {
        let y = viewport_height - 50.0;
        self.rect(10.0, y - 8.0, 264.0, 50.0, [0.045, 0.075, 0.11, 0.98], 1.0);
        self.rect(18.0, y, 92.0, 34.0, [0.08, 0.26, 0.30, 1.0], 1.0);
        self.text(
            if copied { "COPIED" } else { "COPY" },
            18.0 + if copied { 15.0 } else { 24.0 },
            y + 8.0,
            0.86,
            if copied { GREEN } else { CYAN },
            1.0,
        );
        self.rect(116.0, y, 74.0, 34.0, [0.09, 0.13, 0.18, 1.0], 1.0);
        self.text("CLEAR", 128.0, y + 8.0, 0.78, MUTED, 1.0);
        self.text(&format!("{count} CH"), 202.0, y + 8.0, 0.72, MUTED, 1.0);
    }
    ...
    fn block(&mut self, block: &Block, font_scale: f32) {
        let x = 34.0;
        match block {
            ...
            Block::CodeBlock {
                ...
                let height =
                    measure_code_text(text, x + 14.0, 22.0 * font_scale, font_scale, self.width)
                        + 31.0 * font_scale;
                self.rect(
                    x,
                    self.y - 12.0,
...
fn measure_code_text(text: &str, origin: f32, mut y: f32, scale: f32, width: f32) -> f32 {
    ...
    let limit = width - 30.0;
    for c in text.chars() {
        let advance = font::mono_advance(c, scale);
        if c == '\n' || (x + advance > limit && x > origin) {
            x = origin;
            y += line;
            if c == '\n' {
