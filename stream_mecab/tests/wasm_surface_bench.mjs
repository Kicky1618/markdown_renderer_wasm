...
for (let i = 0; i < 1000; i++) for (const chunk of chunks) mecab.appendSurfaces(chunk);
const rounds = 20_000;
const start = performance.now();
let pushed = 0;
for (let i = 0; i < rounds; i++) {
  for (const chunk of chunks) pushed += mecab.appendSurfaces(chunk).push.length;
}
const ms = performance.now() - start;
const appends = rounds * chunks.length;
console.log(`wasm+js surface-only: ${appends} appends in ${ms.toFixed(3)} ms (${(appends / ms / 1000).toFixed(3)} M append/s), pushed=${pushed}`);
mecab.destroy();
