...
Replay determinism also verifies the streaming shape itself. The fourth `timeline` axis compares bounded milestone structure (`chunk`, cumulative decoded characters, parser block count, generated-UI count and event kind), while deliberately ignoring wall-clock timing and semantic commit labels because replay is accelerated and Review policy may differ. A verified replay therefore reports `source ✓ · semantic ✓ · state ✓ · timeline ✓`; changing only Review policy produces `state ✗ · timeline ✓` before Apply, proving the incremental parser/UI emergence path remained identical.
