...
struct MacroNamespace {
    // Built-ins never change after initialization. Parser-local definitions live
    // in a tiny overlay so environments such as matrix can define \cr without
    // cloning the entire built-in macro table.
    builtins: Arc<HashMap<String, MacroDefinition>>,
...
impl MacroNamespace {
    ...
    fn new() -> Self {
        static BUILTINS: OnceLock<Arc<HashMap<String, MacroDefinition>>> = OnceLock::new();
        Self {
            builtins: BUILTINS.get_or_init(build_builtin_macros).clone(),
            current: HashMap::new(),
            group_stack: Vec::new(),
        }
    }
...
fn build_builtin_macros() -> Arc<HashMap<String, MacroDefinition>> {
    // Build the exact pre-existing macro namespace once, then share it between
    // parsers. Static text macros are also tokenized once; cloning their token
    // vectors is equivalent to the old per-expansion lexing but much cheaper.
    let depth_budget = DepthBudget::new(MAX_INPUT_DEPTH);
    let mut expander = MacroExpander {
        lexer: Lexer::new(""),
        mode: Mode::Math,
        stack: Vec::new(),
        macros: MacroNamespace::empty(),
        expansion_count: 0,
        max_expand: 1000,
        depth_budget,
    ...
    for definition in expander.macros.current.values_mut() {
        if let MacroDefinition::Text(text) = definition {
            let (tokens, num_args) = tokenize_text_macro(text);
            *definition = MacroDefinition::Tokens { tokens, num_args };
        }
    }

    Arc::new(expander.macros.current)
}

fn tokenize_text_macro(text: &str) -> (Vec<Token>, usize) {
    let mut num_args = 0;
...
#[cfg(test)]
mod shared_builtin_tests {
    use super::*;

    #[test]
    fn parser_local_macro_override_does_not_leak_between_instances() {
        let mut first = MacroExpander::new("", Mode::Math);
        first.set_text_macro("\\streamdownLocal", "x");
        assert!(first.get_macro("\\streamdownLocal").is_some());

        let second = MacroExpander::new("", Mode::Math);
        assert!(second.get_macro("\\streamdownLocal").is_none());
        assert!(second.get_macro("\\neq").is_some());
    }

    #[test]
    fn grouped_builtin_override_restores_shared_definition() {
        let mut expander = MacroExpander::new("", Mode::Math);
        let before = format!("{:?}", expander.get_macro("\\neq").unwrap());

        expander.begin_group();
        expander.set_text_macro("\\neq", "x");
        assert_ne!(
            format!("{:?}", expander.get_macro("\\neq").unwrap()),
            before
        );
        expander.end_group();
