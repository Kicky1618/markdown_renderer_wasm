...
- 2026-08-31 ChatGPT(math-fastpath): append-only DisplayList の high-res raster を `(display, scale_key)` ごとに1本だけ保持し、item prefix/高さ/幅が安全なとき suffix items のみ描画する incremental path を追加。capacity grow 時は旧prefixを新surfaceへ再描画して edge clip を復元し、tiny-skia が 8191px 超で fill_rect の raster mode を切り替える境界は full render へ fallback。exact-first 4系列（x+x/indexed/text/sqrt）で active pixel mismatch=0、恒久回帰は通常capacity growと8191px境界を固定。current-source A/B 7-run interleaved median は128 streaming-prefix raster `49.440→24.106ms`（2.05x、約51.2%短縮）、cold `0.053→0.056ms/op`。webapp全release tests pass（math 7/7）、wasm32 release check pass。

- 2026-09-01 ChatGPT(bencher-ci): Bencher CIを実運用化。GitHub-hosted runnerはCPU型番/vCPU/arch別testbed、mainの10%低下は履歴alertのみ、PRは同一runner上でbase/headを交互7回測定して10%低下をblocking。意図的50%退行PRで5 syntax項目すべてalert/failureを実証。syntax benchを独立軽量crate化しmain runを約93s→44s、binary cache hitで22sまで短縮。さらにGit tree由来のbenchmark input hashがbase/headで同一ならbuild・測定・Bencher送信を全skipする方針を導入。workflow_dispatchは常に再測定。
2026-09-02 ChatGPT(bencher-ci) UPDATE: benchmark suite sharding verified in PR #4 (parser: 6 metrics only) and PR #5 (syntax: 5 metrics only); both close/archive paths passed. Parallelized independent parser/syntax Cargo builds for dual-suite cold runs; measurement remains serial to avoid benchmark interference.
2026-09-02 ChatGPT(bencher-ci) UPDATE: added adaptive benchmark sampling: minimum 3 samples, stop early when every changed metric has relative MAD <= 1.5%, otherwise continue to max 7; paired PR base/head always use equal sample counts. Synthetic stable/unstable/outlier checks passed.
- 2026-09-02: Benchmarks workflow manual dispatch now supports `suite=all|parser|syntax`; adaptive sampling and suite-specific Bencher metrics remain unchanged.
